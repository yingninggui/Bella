...
NSString *str = @"spotify:user:22twkvspkih7nwqom5dlty3hi:playlist:5g4u6my7planc05uGHcHr1";
...
-(IBAction)fastForward:(id)sender {
    printf("asdjasdfaklsjdflsjdflksdjflksdfjalsdkfjasldfkjasldkfjkasda\n\n\n");
    [self.player playSpotifyURI:str startingWithIndex:beginningIndex startingWithPosition:10 callback:^(NSError *error) {
        if (error != nil) {
...
- (IBAction)seekValueChanged:(id)sender {
    ...
    //printf("%lu\n", (unsigned long)dest);
    if (self.progressSlider.value > .80){
        printf("asdjasdfaklsjdflsjdflksdjflksdfjalsdkfjasldfkjasldkfjkasda\n\n\n");
        [self.player playSpotifyURI:str startingWithIndex:beginningIndex startingWithPosition:10 callback:^(NSError *error) {
            if (error != nil) {
                NSLog(@"*** failed to play: %@", error);
...
- (void)audioStreamingDidLogin:(SPTAudioStreamingController *)audioStreaming {
    [self updateUI];
    [self.player playSpotifyURI:str startingWithIndex:0 startingWithPosition:10 callback:^(NSError *error) {
        if (error != nil) {
            NSLog(@"*** failed to play: %@", error);
            return;
        }
